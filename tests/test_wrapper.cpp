...
template <typename View> struct ParallelForRangeFunctor {
  ...
  KOKKOS_FUNCTION void operator()(int const i) const { mDataV(i) = i; }
};
...
void test_parallel_for_range_free_function_twice(
  ...
  dynk::wrap(
      isExecutedOnDevice,
      [&]() {
        doParallelForFreeFunction<
            Kokkos::DefaultExecutionSpace,
            typename Kokkos::DefaultExecutionSpace::memory_space>(dataDV);
      },
      [&]() {
        doParallelForFreeFunction<
            Kokkos::DefaultHostExecutionSpace,
            typename Kokkos::DefaultHostExecutionSpace::memory_space>(dataDV);
      });
  ...
  dynk::wrap(
      isExecutedOnDevice, [&]<typename ExecutionSpace, typename MemorySpace>() {
        Kokkos::parallel_reduce("label",
                                Kokkos::RangePolicy<ExecutionSpace>(0, 10),
                                ParallelReduceRangeFunctor(), value);
      });
...
void doParallelReduceFreeFunction(int &value) {
  Kokkos::parallel_reduce(
      "label", Kokkos::RangePolicy<ExecutionSpace>(0, 10),
      KOKKOS_LAMBDA(int const, int &valueLocal) { valueLocal++; }, value);
}
...
void test_parallel_reduce_range_free_function_twice(
    bool const isExecutedOnDevice) {
  int value = 0;
  dynk::wrap(
      isExecutedOnDevice,
      [&]() {
        doParallelReduceFreeFunction<
            Kokkos::DefaultExecutionSpace,
